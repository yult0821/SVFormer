...
#             # add noise to inputs, testing model robustness, 20240606
#             save_dir = '/userhome/transformer/UniFormer/video_classification/exp/uniformer2d_psnn_tri57_base45_s16b_lgf_lplif_hdlif_ucf101_scratch/'
#             if not os.path.exists(save_dir):
#                 os.makedirs(save_dir)
#                 print(f"Directory created at {save_dir}")
#             file_path = os.path.join(save_dir, 'model_inf_inputs_eg.pt')
#             torch.save(inputs, file_path)
#             print(f"Saved model_inf_inputs_eg at {file_path}")
        
#             print('type(inputs): ', type(inputs), len(inputs)) #type(inputs):  <class 'list'> 1
#             print('inputs[0].shape: ', inputs[0].shape)
#             print('mean and std of inputs:', inputs[0].mean(), inputs[0].std()) #mean and std of inputs: tensor(-0.3524, device='cuda:0') tensor(1.1409, device='cuda:0')
#             print('max and min of inputs:', inputs[0].max(), inputs[0].min()) #max and min of inputs: tensor(2.4444, device='cuda:0') tensor(-2., device='cuda:0')
#             exit(0)
            
            # add noise to inputs, testing model robustness, 20240606
            for i in range(len(inputs)):
                # gaussian noise
                #add_ns = torch.randn(inputs[i].shape).cuda(non_blocking=True) * (inputs[i].std()*0.5) # gaussian noise
                #add_ns = torch.randn(inputs[i].shape).cuda(non_blocking=True) * (inputs[i].std()*0.1) # gaussian noise
                #add_ns = torch.randn(inputs[i].shape).cuda(non_blocking=True) * (inputs[i].std()*1) # gaussian noise
                #add_ns = torch.randn(inputs[i].shape).cuda(non_blocking=True) * (inputs[i].std()*0.3) # gaussian noise
                #inputs[i] = inputs[i] + add_ns #.cuda(non_blocking=True)
                
                # salt-and-pepper noise
                sp_prob = 0.3 #0.2 #0.1
                ori_in = inputs[i].cpu()
                ns_in = salt_and_pepper(ori_in, sp_prob)
                inputs[i] = ns_in.cuda(non_blocking=True)
        ...
        # reset the model states when using snn (added by yult 0529)
...
#     # calculate energy consumption 2024.5.11
#     import time
#     ts1 = time.time()
