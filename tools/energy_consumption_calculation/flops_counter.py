...
# ssa_info = {'depth': 8, 'Nheads': 6, 'embSize': 384, 'patchSize': 14, 'Tsteps': 4}  # lifconvbn-6-384
...
def get_energy_cost(model, ssa_info):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, (nn.Linear, nn.Conv1d, nn.Conv2d)):  # obtain same results
            ...
            if 'localblock' in name:
                name_split = name.split('.', 2)
                name = f'localblock[{name_split[1]}].{name_split[2]}'
                # name = f'{name_split[0]}[{int(name_split[1])}].{name_split[2]}'
            if 'blocks' in name:
                name_split = name.split('.', 2)
                name = f'blocks[{name_split[1]}].{name_split[2]}'
            print(name)
    ...
    # for localSSA
    localSSA_Nac_base = Tsteps * Nheads * pow(patchSize/2, 2) * embSize_per_head * embSize_per_head * 4  # same to SSA_Nac_base, may show advantage under traditional DNNs
    q_lif_r = eval(f'model.localblock[0].local2_SSA.q_conv.lif.accumulated_syops_cost[3]') / 100
    k_lif_r = eval(f'model.localblock[0].local2_SSA.k_conv.lif.accumulated_syops_cost[3]') / 100
    v_lif_r = eval(f'model.localblock[0].local2_SSA.v_conv.lif.accumulated_syops_cost[3]') / 100
    qkv_fr.append([q_lif_r, k_lif_r, v_lif_r])
    # calculate the number of ACs for Q*K*V matrix computation
    tNac = localSSA_Nac_base * (min(k_lif_r, v_lif_r) + q_lif_r)
    Nac += tNac
    
    # for globalSSA
    for d in range(depth-2): # [0-5]            
        q_lif_r = eval(f'model.blocks[{d}].attn.q_lif.accumulated_syops_cost[3]') / 100
        k_lif_r = eval(f'model.blocks[{d}].attn.k_lif.accumulated_syops_cost[3]') / 100
        v_lif_r = eval(f'model.blocks[{d}].attn.v_lif.accumulated_syops_cost[3]') / 100
        qkv_fr.append([q_lif_r, k_lif_r, v_lif_r])
        # calculate the number of ACs for Q*K*V matrix computation
