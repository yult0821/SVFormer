...
def get_syops_pytorch(model, input_res, dataloader=None,
    ...
    if dataloader is not None:
        ...
        batch_idx = 0
        for batch, target, _, _ in dataloader:
            batch_idx += 1

            torch.cuda.empty_cache()

            batch = batch[0].float().to(next(syops_model.parameters()).device)  # torch.Size([128, 3, 32, 32])
            target = target.float().to(next(syops_model.parameters()).device) # 2023.2.23
...
def print_model_with_syops(model, total_syops, total_params, syops_units='GMac',
    ...
    def syops_repr(self):
        ...
        accumulated_syops_cost[0] /= model.__batch_counter__  # 取均值 bs*iterations
        accumulated_syops_cost[1] /= model.__batch_counter__
        accumulated_syops_cost[2] /= model.__batch_counter__
        accumulated_syops_cost[3] /= model.__times_counter__  # 取均值 iterations
