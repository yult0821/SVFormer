...
# 1102 try all BNT
TN = 16  # cfg.UNIFORMER.T   cfg.DATA.NUM_FRAMES
...
class QKAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.in_lif = act_layer()
        self.qk = layer.Linear(dim, dim * 2) # bias=qkv_bias (0818)
        
        # add batchnorm 0818 before input to lif  (0830: try to only use bn to q k v)
        self.norm1 = bn_1d(dim * 2 * TN) # 1023 try all BN
        # self.norm1 = norm_layer(dim * 3)  # 1013 try layernorm in ssa
...
#         self.proj_drop = layer.Dropout(proj_drop)  # SNN, no need to dropout? 0829
    ...
    def forward(self, x):
        ...
        # add batchnorm 0818 before input to lif  (0830: try to only use bn to q k v)
        qk_ = self.norm1(self.qk(x).transpose(-2, -1)).transpose(-2, -1).flatten(0, 1)
        # qkv_ = self.norm1(self.qkv(x)).flatten(0, 1)   # 1013 try layernorm in ssa
        
        qk = qk_.reshape(T*B, N, 2, self.num_heads, torch.div(C, self.num_heads, rounding_mode='trunc')).permute(2, 0, 3, 1, 4)
        q, k = qk[0], qk[1]
...
#         q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple) # torch.Size([1, 5, 1568, 64])
        ...
        # QK attention
        # m = self.m_lif(torch.sum(q_sk, 4)) # mask, sum over the channel dimension
        # m = torch.unsqueeze(m, 4)
        m = self.m_lif(torch.sum(q_sk, 4, keepdim=True)) # mask, sum over the channel dimension
#         fr = (m.sum() / m.numel()).item()
#         print("QKAttn musk firing rate", fr)
#         print("QKAttn musk shape", m.shape)  # (T, B, Nheads, N, C/Nheads)
#         print("QKAttn musk m[0,0,0,:,:]", m[0,0,0,:20,:])
        x = k_sk.masked_fill(~m.bool(), 0) # 0-1 binary x
#         print("QKAttn k_sk shape", k_sk.shape)  # (T, B, Nheads, N, C/Nheads)
#         print("QKAttn k_sk k_sk[0,0,0,:,:]", k_sk[0,0,0,:20,:20])
#         print("QKAttn musked_k shape", x.shape)  # (T, B, Nheads, N, C/Nheads)
#         print("QKAttn musked_k x[0,0,0,:,:]", x[0,0,0,:20,:20])
        x = x.transpose(2, 3).reshape(T, B, N, C).contiguous()  # (T, B, N, C)
...
class Uniformer2d_psnn_convqkf(nn.Module): # uniformer2d_psnn_try28_allBNT_headV2.py
    """ Vision Transformer
...
#         self.norm = bn_2d(embed_dim[-1])  # no need? 1016
        ...
        self.dwconv_bf_head = nn.Conv3d(embed_dim[-1], embed_dim[-1], (cfg.DATA.NUM_FRAMES, img_size // 32, img_size // 32), groups=embed_dim[-1])
