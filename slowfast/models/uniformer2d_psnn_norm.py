...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # 0821: try batchnorm before and after ssa
    ...
    def forward(self, x):
        ...
        x = xpre1.flatten(3).transpose(2, 3) + self.drop_path(self.attn(x)) # (T B N C)
        xpre2 = x
        x = self.norm2(x.transpose(2, 3)).transpose(2, 3) # (T B N C)
        x = xpre2 + self.drop_path(self.mlp(x)) # (T B N C)
...
class Uniformer2d_psnn_norm(nn.Module):
    """ Vision Transformer
