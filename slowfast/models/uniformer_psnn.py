...
def conv_3xnxn(inp, oup, kernel_size=3, stride=3, groups=1):
    return layer.Conv3d(inp, oup, (3, kernel_size, kernel_size), (2, stride, stride), (1, 1, 1), groups=groups, step_mode='m')
...
# def act_layer():
#     return neuron.LIFNode(surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='cupy') # backend='cupy' 'torch'
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=act_layer, drop=0.):
        ...
        self.norm1 = norm_layer(hidden_features)  # 0914 added
#         self.drop1 = layer.Dropout(drop)
        self.act2 = act_layer()
        self.fc2 = layer.Linear(hidden_features, out_features)
        self.norm2 = norm_layer(out_features)  # 0914 added
#         self.drop2 = layer.Dropout(drop)

    def forward(self, x):
        x = self.act1(x)
        x = self.fc1(x)
        x = self.norm1(x)  # 0914 added
#         x = self.drop1(x)
        x = self.act2(x)
        x = self.fc2(x)
        x = self.norm2(x)  # 0914 added
#         x = self.drop2(x)
        return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # add batchnorm 0914 before input to lif  (only use bn to q k v)
        #self.norm1 = bn_1d(dim * 3)
        self.norm1 = nn.LayerNorm(dim * 3)
...
#         self.proj_drop = layer.Dropout(proj_drop)
    ...
    def forward(self, x):
        ...
        qkv_ = self.norm1(self.qkv(x)).flatten(0, 1)
...
#         # add batchnorm 0914 before input to lif   (bad effects, verified in unif2d_psnn)
...
class CMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=act_layer, drop=0.):
        ...
        self.norm1 = bn_3d(hidden_features)  # 0914 added
#         self.drop1 = layer.Dropout(drop)
        self.act2 = act_layer()
        self.fc2 = conv_1x1x1(hidden_features, out_features)
        self.norm2 = bn_3d(out_features)  # 0914 added
#         self.drop2 = layer.Dropout(drop)
...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.act2 = act_layer()
        self.attn = conv_5x5x5(dim, dim, groups=dim) # not change size
        self.act3 = act_layer()
...
class SABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(2, 3).reshape(T, B, C, D, H, W) # torch.Size([1, 320, 8, 14, 14])
        return x    
...
class SpeicalPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # 0914, to use overlapping conv3d in patch embedding
        self.proj = conv_3xnxn(in_chans, embed_dim, kernel_size=patch_size[0]+2, stride=patch_size[0])
        #self.proj = conv_3xnxn(in_chans, embed_dim, kernel_size=patch_size[0], stride=patch_size[0])
        
        # use batchnorm instead 0914
        # self.norm = nn.LayerNorm(embed_dim)
        self.norm = bn_3d(embed_dim)
...
#         x = self.norm(x)
#         x = x.reshape(T, B, D, H, W, -1).permute(0, 1, 5, 2, 3, 4).contiguous()
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, std=False):
        ...
        if std:
            ...
            self.proj = conv_1xnxn(in_chans, embed_dim, kernel_size=patch_size[0]+2, stride=patch_size[0]) # 0914, to use overlapping conv2d in patch embedding
    ...
    def forward(self, x):
        ...
        # use batchnorm instead 0914
        x = self.norm(x)
#         T, B, C, D, H, W = x.shape # [1, 128, 8, 28, 28]
...
#         x = x.reshape(T, B, D, H, W, -1).permute(0, 1, 5, 2, 3, 4).contiguous() # torch.Size([1, 8, 28, 28, 128]) -> torch.Size([1, 128, 8, 28, 28])
        return x


@MODEL_REGISTRY.register()
class Uniformer_psnn(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, cfg):
        ...
        self.T = cfg.UNIFORMER.T # added 2023.7.5 (time steps for uniformer_snn)
        ...
        if split:
            ...
            self.blocks4 = nn.ModuleList([
                SABlock(
                    ...
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], norm_layer=norm_layer)
            for i in range(depth[3])])
        #self.norm = bn_3d(embed_dim[-1])  # there is a layernorm in the last SABlock, i.e. Lif-Linear-Layernorm, just before this one (0914)
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks4):
            if self.use_checkpoint and i < self.checkpoint_num[3]:
                ...
                x = blk(x)
        # there is a layernorm in the last SABlock, i.e. Lif-Linear-Layernorm, just before this one (0914)
        #x = self.norm(x)  # torch.Size([1, 512, 8, 7, 7]) -> torch.Size([1, 512, 8, 7, 7])
        x = self.pre_logits(x)
        return x

    def forward(self, x):
        # x = x[0] # tuple/list -> tensor (B, C, F, H, W) torch.Size([1, 3, 16, 224, 224])
        ...
        x = self.forward_features(x) # torch.Size([1, 512, 8, 7, 7])
        x = x.flatten(3).mean(-1) # torch.Size([1, 512])
