...
class BatchNorm3d_eT(nn.BatchNorm3d):
    def __init__(
            self,
            num_features,
            eps=1e-5,
        ...
        """
        将T与C维度合并, 再利用nn.BatchNorm3d操作, 相当于每个时间步单独计算均值和方差。输入应为[T, B, C, D, H, W], 多步模式下应用。单步模式用nn.BatchNorm3d即可
    ...
    def forward(self, x: Tensor):
        if x.dim() != 6:
            raise ValueError(f'expected x with shape [T, B, C, D, H, W], but got x with shape {x.shape}!')
        T, B, C, D, H, W = x.shape
        x = x.permute(1, 0, 2, 3, 4, 5).contiguous()
        x = x.reshape(B, T*C, D, H, W).contiguous()
        x = super().forward(x)
        x = x.reshape(B, T, C, D, H, W).contiguous()
        x = x.permute(1, 0, 2, 3, 4, 5).contiguous()
        return x
...
def conv_3xnxn(inp, oup, kernel_size=3, stride=3, groups=1):
    return layer.Conv3d(inp, oup, (3, kernel_size, kernel_size), (1, stride, stride), (1, 1, 1), groups=groups, step_mode='m')

def conv_1xnxn(inp, oup, kernel_size=3, stride=3, groups=1):
    return layer.Conv3d(inp, oup, (1, kernel_size, kernel_size), (1, stride, stride), (0, 1, 1), groups=groups, step_mode='m')

# def conv_3xnxn_std(inp, oup, kernel_size=3, stride=3, groups=1): # spatio-temporal downsampling
...
def conv_1x1x1(inp, oup, groups=1):
    return layer.Conv3d(inp, oup, (1, 1, 1), (1, 1, 1), (0, 0, 0), groups=groups, step_mode='m')

def conv_3x3x3(inp, oup, groups=1):
    return layer.Conv3d(inp, oup, (3, 3, 3), (1, 1, 1), (1, 1, 1), groups=groups, step_mode='m')

def conv_3x5x5(inp, oup, groups=1): # 第一个维度取决于3d-clip输入的长度，即单次输入的帧数，最少为3
    return layer.Conv3d(inp, oup, (3, 5, 5), (1, 1, 1), (1, 2, 2), groups=groups, step_mode='m')

def conv_5x5x5(inp, oup, groups=1): # 第一个维度取决于3d-clip输入的长度，即单次输入的帧数，最少为5
    return layer.Conv3d(inp, oup, (5, 5, 5), (1, 1, 1), (2, 2, 2), groups=groups, step_mode='m')
...
def bn_3d(dim):
    return BatchNorm3d_eT(dim) # 将T与C维度合并, 再利用nn.BatchNorm3d操作, 相当于每个时间步单独计算均值和方差
    # return layer.BatchNorm3d(dim, step_mode='m')
...
# 1102 try all BNT
# 在使用layer.BatchNorm?d时，需将TN设为1
TN = 4 #4 #16  # cfg.UNIFORMER.T   cfg.DATA.NUM_FRAMES
...
class CMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=act_layer, drop=0.):
        ...
        self.norm1 = bn_3d(hidden_features * TN)  # 0901 added
#         self.drop1 = layer.Dropout(drop)  # SNN, no need to dropout? 0829
        self.act2 = act_layer()
        self.fc2 = conv_1x1x1(hidden_features, out_features)
        self.norm2 = bn_3d(out_features * TN)  # 0901 added
#         self.drop2 = layer.Dropout(drop)  # SNN, no need to dropout? 0829
...
#         self.norm1 = bn_2d(dim * TN)
        ...
        self.conv1 = conv_1x1x1(dim, dim, 1) # not change size
#         self.act2 = act_layer()
        self.attn = conv_3x5x5(dim, dim, groups=dim) # not change size
#         self.act3 = act_layer()
        self.conv2 = conv_1x1x1(dim, dim, 1) # not change size
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = bn_3d(dim * TN)
...
#         x = x + self.norm1(self.pos_embed(self.in_lif(x)))
        ...
        T, B, C, D, H, W = x.shape
        x = x.flatten(3).transpose(2, 3) # (T B N C) N=D*H*W
        x = x + self.drop_path(self.attn(x)) # (T B N C)
        x = x + self.drop_path(self.mlp(x)) # (T B N C)
        x = x.transpose(2, 3).reshape(T, B, C, D, H, W)
...
class SpeicalPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # self.in_lif = act_layer()
        self.proj = conv_3xnxn(in_chans, embed_dim, kernel_size=patch_size[0]+2, stride=patch_size[0])# 0904, to use overlapping conv2d in patch embedding
#         self.proj = conv_nxn(in_chans, embed_dim, kernel_size=patch_size[0], stride=patch_size[0])
        
        # use batchnorm instead 0818
        # self.norm = nn.LayerNorm(embed_dim)
        self.norm = bn_3d(embed_dim * TN)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, std=False):
        ...
        self.in_lif = act_layer()
#         self.proj = conv_nxn(in_chans, embed_dim, kernel_size=patch_size[0], stride=patch_size[0])
        self.proj = conv_1xnxn(in_chans, embed_dim, kernel_size=patch_size[0]+2, stride=patch_size[0]) # 0904, to use overlapping conv2d in patch embedding
        # if std:
...
class Uniformer3d_psnn_lgf(nn.Module): # uniformer2d_psnn_try28_allBNT_headV2.py
    """ Vision Transformer
...
#         self.norm = bn_2d(embed_dim[-1])  # no need? 1016
        ...
        self.lpDWConv = layer.Conv3d(embed_dim[2], embed_dim[2], kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1), groups=embed_dim[2], step_mode='m')
        self.lpPWConv = layer.Conv3d(embed_dim[2], embed_dim[-1], kernel_size=(1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0), groups=1, step_mode='m')
        self.lpBN = bn_3d(embed_dim[-1] * TN)
        ...
        self.lif_bf_head = act_layer() # (T, B, C, D, H, W) # try add plif before the head 2024.2.29 
        self.dwconv_bf_head = nn.Conv3d(feature_outdim, feature_outdim, (cfg.DATA.NUM_FRAMES, img_size // 32, img_size // 32), groups=feature_outdim) #(B, C, T*D, H, W)
...
#         x = self.norm(x)  # no need? 1016  # torch.Size([1, 512, 8, 7, 7]) -> torch.Size([1, 512, 8, 7, 7])
    ...
    def forward(self, x):
        ...
        # V2: try to use the temporal info in the classification head, not just average them 1031
        x = self.forward_features(x) # tensor (T, B, C, D, H, W)
        # print(x.shape)
        ...
        T, B, C, D, H, W = x.shape
        x = x.permute(1, 2, 0, 3, 4, 5).contiguous() # (T, B, C, D, H, W) -> (B, C, T, D, H, W)
        x = x.reshape(B, C, F, H, W).contiguous()
