...
class Uniformer2d_psnn_head(nn.Module): # uniformer2d_psnn_try28_allBNT_headV2.py
    """ Vision Transformer
...
#         x = self.norm(x)  # no need? 1016  # torch.Size([1, 512, 8, 7, 7]) -> torch.Size([1, 512, 8, 7, 7])
    ...
    def forward(self, x):
        ...
        # V0: original version - average
        x = self.forward_features(x) # tensor (T, B, C, H, W)
        # print(x.shape)
        x = x.flatten(3).mean(-1) # torch.Size([B, C])
        # print(x.shape)
        x = self.head(x.mean(0)) # torch.Size([B, 174])
        # print(x.shape)
