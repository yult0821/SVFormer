...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # self.in_lif = act_layer() # 1016 convBnLif
        ...
        self.norm1 = bn_1d(dim * 3) # 1023 try all BN
...
#         self.norm2 = bn_1d(dim)
        # self.proj_lif = act_layer() # 1016 convBnLif
        self.ssa_lif = act_layer() # 1024 convBnLif
        self.proj = layer.Linear(dim, dim)
        
        # 1013: add layernorm after proj
        # self.norm2 = norm_layer(dim)
        self.norm2 = bn_1d(dim) # 1023 try all BN
...
#         self.proj_drop = layer.Dropout(proj_drop)  # SNN, no need to dropout? 0829
    ...
    def forward(self, x):
        ...
        # x = self.in_lif(x) # 1016 convBnLif
        T, B, N, C = x.shape

        # add batchnorm 0818 before input to lif  (0830: try to only use bn to q k v)
        qkv_ = self.norm1(self.qkv(x).transpose(-2, -1)).transpose(-2, -1).flatten(0, 1) # 1023 try all BN
...
#         x = self.norm2(x.transpose(-2, -1)).transpose(-2, -1)
        ...
        # x = self.proj_lif(x) # 1016 convBnLif
        x = self.ssa_lif(x) # 1024 convBnLif
...
class CMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=act_layer, drop=0.):
        ...
        self.norm1 = bn_2d(hidden_features)  # 0901 added
        self.act1 = act_layer() # 1016 convBnLif
#         self.drop1 = layer.Dropout(drop)  # SNN, no need to dropout? 0829
        # self.act2 = act_layer() # 1016 convBnLif
        self.fc2 = conv_1x1(hidden_features, out_features)
        self.norm2 = bn_2d(out_features)  # 0901 added
        self.act2 = act_layer() # 1016 convBnLif
#         self.drop2 = layer.Dropout(drop)  # SNN, no need to dropout? 0829
...
#         x = x + self.drop_path(self.mlp(x))
        ...
        # 1016 convBnLif
        x = x + self.in_lif(self.norm1(self.pos_embed(x)))
        x = x + self.act1(self.norm2(self.conv2(self.attn(self.conv1(x)))))
        x = x + self.mlp(x)
...
#         x = x.transpose(2, 3).reshape(T, B, C, H, W) # torch.Size([1, 320, 8, 14, 14])
        ...
        T, B, C, H, W = x.shape
        x = x.flatten(3).transpose(2, 3) # (T B N C) N=H*W
        x = x + self.attn(x) # (T B N C)
        x = x + self.mlp(x) # (T B N C)
        x = x.transpose(2, 3).reshape(T, B, C, H, W)
...
class Uniformer2d_psnn_convBnLif(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, cfg):
        ...
        # add lif before head  1016  
        #self.norm = bn_2d(embed_dim[-1])
        # self.lif_head = act_layer() # no need in convBnLif case? 1016
    ...
    def forward_features(self, x):
        ...
        # x = self.lif_head(x) # no need in convBnLif case? 1016
    ...
    def forward(self, x):
        ...
        x = self.forward_features(x) # torch.Size([T, 1, 512, 8, 7, 7]) (T, B, C, H, W)
        # print(x.shape)
        x = x.flatten(3).mean(-1) # torch.Size([T, 1, 512]) (T, B, C)
        # print(x.shape)
        x = self.head(x.mean(0)) # torch.Size([1, 174]) (B, #class)
        # print(x.shape)
        return x
