...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=act_layer, drop=0.):
        ...
        self.norm1 = norm_layer(hidden_features)  # 0901 added
    ...
    def single_step_forward(self, x):
        if self.y is None:
            if self.dwconv is None:
                ...
                h = (x.size(2) + 2 - (3 - 1) - 1) // 2 + 1 # ⌊ (Hin​+2×padding[0]−dilation[0]×(kernel_size[0]−1)−1) / stride[0] ​+ 1 ⌋
...
class Uniformer2d_prsnn(nn.Module):
    """ Vision Transformer
        ...
        if split:
            ...
            self.blocks4 = nn.ModuleList([
                SABlock(
                    ...
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], norm_layer=norm_layer)
            for i in range(depth[3])])
        # self.norm = bn_2d(embed_dim[-1])  # there is a layernorm in the last SABlock, i.e. Lif-Linear-Layernorm, just before this one (0911)
        
        # rnn layer (0911)
        recConvBn = nn.Sequential(
            layer.Conv2d(embed_dim[-1], embed_dim[-1], (3, 3), (1, 1), (1, 1), groups=embed_dim[-1], step_mode='s'),
            layer.BatchNorm2d(embed_dim[-1], step_mode='s')
        )
        self.reclayer = ConvRecurrentContainer(recConvBn, in_channels=embed_dim[-1], out_channels=embed_dim[-1], stride=1)
