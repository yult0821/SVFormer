...
class Uniformer3d_psnn(nn.Module): # uniformer2d_psnn_try28_allBNT_headV2.py
    """ Vision Transformer
...
#         self.norm = bn_2d(embed_dim[-1])  # no need? 1016
        ...
        self.dwconv_bf_head = nn.Conv3d(embed_dim[-1], embed_dim[-1], (cfg.DATA.NUM_FRAMES, img_size // 32, img_size // 32), groups=embed_dim[-1]) #(B, C, T*D, H, W)
        self.dwconv_bn = nn.BatchNorm3d(embed_dim[-1])
        # self.pre_logits = nn.Identity()
