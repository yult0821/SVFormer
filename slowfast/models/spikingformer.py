...
model_path = {
    ...
    'spikingformer_8x384_in1k': os.path.join(model_path, 'spikingformer_8x384_in1k.pth'),
...
class SPS(nn.Module):
    ...
    def forward(self, x):
        ...
        # without patch embedding + position embedding  (yult, 0419)
...
class Spikingformer(nn.Module):
    def __init__(self, cfg
        ...
        depth = cfg.SPIKINGFORMER.DEPTH
        patch_size = cfg.SPIKINGFORMER.PATCH_SIZE
        embed_dims = cfg.SPIKINGFORMER.EMBED_DIM
        num_heads = cfg.SPIKINGFORMER.NUM_HEADS
        mlp_ratios = cfg.SPIKINGFORMER.MLP_RATIO
        qkv_bias = cfg.SPIKINGFORMER.QKV_BIAS
        qk_scale = cfg.SPIKINGFORMER.QKV_SCALE
        drop_rate = cfg.SPIKINGFORMER.DROPOUT_RATE
        attn_drop_rate = cfg.SPIKINGFORMER.ATTENTION_DROPOUT_RATE
        drop_path_rate = cfg.SPIKINGFORMER.DROP_DEPTH_RATE
        sr_ratios = cfg.SPIKINGFORMER.SR_RATIOS
    ...
    def get_pretrained_model(self, cfg):
        if cfg.SPIKINGFORMER.PRETRAIN_NAME:
            checkpoint = torch.load(model_path[cfg.SPIKINGFORMER.PRETRAIN_NAME], map_location='cpu')
